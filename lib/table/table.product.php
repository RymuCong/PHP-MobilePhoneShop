...
function productAdd($data)
{
	...
	$price_sale      = (float)$data['price_sale'];    //gia sale
...
// 	";
	...
	$sql = "
		SELECT 
            p.product_id, p.model, p.name, p.image, p.manufacturer_id, p.price, p.price_sale, p.sort_order, p.status, p.date_added, p.description, p.best_seller, p.featured, p.tag,
			m.name AS manufacturer,
		FROM `product` AS p
...
// 	";
	...
	if ( is_array($rs) && !empty($rs) )
	{
		...
		return array(
			...
			'price_sale'        => price_text_vnd_sale($rs['price_sale']),
...
function productFeatureds( $data=array() )
{
	...
	foreach ($rs as &$row) 
	{
	   ...
	   $row['price_text_sale'] = price_text_vnd_sale($row['price_sale']);
...
function RatingAvg($product_id)
{
	$sql = "SELECT AVG(rating) FROM rating WHERE product_id = {$product_id}";
	return round((float)db_one($sql),1);
}