...
function pricesale2($product_id){
	$sql="
		SELECT
		`product_id`,
		`price_sale`
	...
	foreach ($_SESSION['cart'] as $product_id => $quantity) 
	{
	   ...
	   if (is_array($product_info) && !empty($product_info)) 
	   {
            ...
            $cart_data[$product_id] = array(
			     ...
			     'total'           => (int)($product_info['price_sale'] != 0 ? $product_info['price_sale'] * $quantity : $product_info['price'] * $quantity)
		    );
	   } 
...
function cartGetProductsWithFormat()
{
	...
	foreach (cartGetProducts() as $product) 
	{
		...
		$dong = pricesale($product['product_id']);
		$price_sale = number_format($dong['price_sale'],0,'.',',').' ₫';
